...
// FIX #2: back 9 gets its own independent pair from the start
const mkM=(pA,pB)=>({
front:{pA,pB,sA:{},sB:{}},
back:{pA:[…pA],pB:[…pB],sA:{},sB:{}}
});
...
let a=0,b=0,pl=0;
hs.forEach(h=>{
const scr=isFri||isThuScramble(!isFri,h.h);
const va=getScore(m.sA,m.pA,h.h,scr);
const vb=getScore(m.sB,m.pB,h.h,scr);
...
const ldr=diff>0?tA:tB;col=diff>0?CA:CB;
if(rem===0)txt=`${ldr} wins ${Math.max(last.a,last.b)}-${Math.min(last.a,last.b)}`;
else if(Math.abs(diff)>rem)txt=`${ldr} wins ${Math.abs(diff)}&${rem}`;
else txt=`${ldr} ${Math.abs(diff)}UP thru ${played.length}`;
}
return(
...
// FIX #8: show CTP flag in tracker
const topColor=r.ctp?”#c9a227”:r.scr?”#2a4a2a”:”#333”;
return(
<div key={r.h} style={{display:“flex”,flexDirection:“column”,alignItems:“center”,gap:1}}>
<div style={{fontSize:8,color:topColor,width:28,textAlign:“center”,fontWeight:r.ctp||r.scr?700:400}}>
{r.h}{r.ctp?“⚑”:r.scr?”~”:””}
</div>
...
<div style={{width:28,height:11,background:done?wc+“22”:“transparent”,borderRadius:3,border:done?“1px solid “+wc+“44”:“1px solid #111”,display:“flex”,alignItems:“center”,justifyContent:“center”}}>
{done&&<div style={{width:6,height:6,borderRadius:“50%”,background:wc}}/>}
</div>
...
// Shared scorecard cell logic
function ScoreCell({s,p,canEdit,onTap,isEd,v,setV,onCommit,onEsc,teamColor}){
if(canEdit){
if(isEd) return(
<input autoFocus value={v}
onChange={e=>setV(e.target.value.replace(/\D/g,””))}
onBlur={onCommit}
onKeyDown={e=>{if(e.key===“Enter”)onCommit();if(e.key===“Escape”)onEsc();}}
style={{width:26,textAlign:“center”,background:”#111”,color:”#fff”,border:“1.5px solid “+teamColor,borderRadius:3,fontSize:13,fontWeight:700,padding:1,outline:“none”}}/>
);
return(
<div onClick={onTap} style={{cursor:“pointer”,display:“flex”,alignItems:“center”,justifyContent:“center”,minWidth:26,minHeight:26}}>
<Pip s={s} p={p}/>
</div>
);
}
return<div style={{display:“flex”,alignItems:“center”,justifyContent:“center”}}><Pip s={s} p={p}/></div>;
}

function ThuCard({holes,pA,pB,sA,sB,me,onScore}){
const [ed,setEd]=useState(null),[v,setV]=useState(””);
const commit=(p,h,t)=>{
// FIX #5: allow clearing — if empty string tap, set to null via special value 0 means delete
const n=parseInt(v);
if(!isNaN(n)&&n>0) onScore(p,h,n,t,false);
else if(v===””) onScore(p,h,null,t,false);
setEd(null);setV(””);
};
const hw=h=>{
const scr=isThuScramble(true,h.h);
const va=getScore(sA,pA,h.h,scr),vb=getScore(sB,pB,h.h,scr);
if(!va||!vb)return null;return va<vb?“A”:vb<va?“B”:“tie”;
};
...
{holes.map(h=>{
const w=hw(h),wc=w===“A”?CA:w===“B”?CB:w===“tie”?”#555”:“transparent”;
const isScr=isThuScramble(true,h.h);
return<th key={h.h} style={{…th,background:h.ctp?”#1a0f00”:isScr?”#0a1a0a”:w?wc+“18”:”#0a0a0a”,color:h.ctp?”#c9a227”:isScr?”#2d6a4f”:w?wc:”#333”}}>
{h.h}{h.ctp?“⚑”:isScr?”~”:””}
{w&&<div style={{width:5,height:5,borderRadius:“50%”,background:wc,margin:“1px auto 0”}}/>}
</th>;
})}
<th style={{...th}}>Tot</th>
</tr>
...
{holes.map(h=>{
const scr=isThuScramble(true,h.h);
const w=hw(h),va=getScore(sA,pA,h.h,scr),vb=getScore(sB,pB,h.h,scr);
const wc=w===“A”?CA:w===“B”?CB:w===“tie”?”#888”:”#333”;
return<td key={h.h} style={{...td,fontSize:9,fontWeight:700,color:wc}}>{va||”–”}/{vb||”–”}</td>;
})}
<td style={{...td}}/>
</tr>
...
<tbody>
{rows.map(({p,t})=>{
const sc=t===“A”?sA:sB,pt=holes.reduce((s,h)=>s+h.p,0);
const tot=holes.reduce((s,h)=>s+(sc?.[p]?.[h.h]||0),0);
const isMe=p===me;
const getBest=h=>{const scr=isThuScramble(true,h.h);return getScore(sc,t===“A”?pA:pB,h.h,scr);};
return(
...
{holes.map(h=>{
const scr=isThuScramble(true,h.h);
const s=sc?.[p]?.[h.h]??null,isEd=ed?.p===p&&ed?.h===h.h;
const isBest=s!==null&&s===getBest(h);
// FIX #1: on hole 10, only the first player in pair enters (it’s a shared scramble score)
const canEdit=isMe&&(!scr||(t===“A”?pA[0]:pB[0])===p);
return(
<td key={h.h} style={{…td,padding:2,background:isBest?(t===“A”?CA:CB)+“14”:“transparent”}}>
<ScoreCell s={s} p={h.p} canEdit={canEdit}
onTap={()=>{setEd({p,h:h.h});setV(s?.toString()||””);}}
isEd={isEd} v={v} setV={setV}
onCommit={()=>commit(p,h.h,t)}
onEsc={()=>{setEd(null);setV(””);}}
teamColor={t===“A”?CA:CB}/>
</td>
);
})}
...
const [ed,setEd]=useState(null),[v,setV]=useState(””);
const commit=(pair,h,t)=>{
const n=parseInt(v);
if(!isNaN(n)&&n>0) onScore(pair,h,n,t);
else if(v===””) onScore(pair,h,null,t);
setEd(null);setV(””);
};
const hw=h=>{const va=getScore(sA,pA,h.h,true),vb=getScore(sB,pB,h.h,true);if(!va||!vb)return null;return va<vb?“A”:vb<va?“B”:“tie”;};
const th={padding:“4px 2px”,textAlign:“center”,fontSize:9,color:”#444”,borderRight:“1px solid #0a0a0a”,fontWeight:400};
const td={padding:“2px 2px”,textAlign:“center”,borderRight:“1px solid #080808”};
// FIX #3: compare by name membership not reference equality
...
const pt=holes.reduce((s,h)=>s+h.p,0);
const tot=holes.reduce((s,h)=>s+(getScore(sc,pair,h.h,true)||0),0);
return(
...
<ScoreCell s={s} p={h.p} canEdit={isMe}
onTap={()=>{setEd({t,h:h.h});setV(s?.toString()||””);}}
isEd={isEd} v={v} setV={setV}
onCommit={()=>commit(pair,h.h,t)}
onEsc={()=>{setEd(null);setV(””);}}
teamColor={t===“A”?CA:CB}/>
</td>
);
...
<span style={{fontSize:11,color:”#c9a227”,fontWeight:700}}>{side===“front”?“FRONT”:“BACK”} 9 — M{mi+1}</span>
<span onClick={()=>setOpen(false)} style={{color:”#555”,cursor:“pointer”,fontSize:18}}>✕</span>
</div>
{[“A”,“B”].map(t=>{
...
// FIX #6: snake draft order tracker
const totalPicked=st.pA.length+st.pB.length-2; // minus 2 for auto-assigned captains
...
const nextCaptain=nextTeam===“A”?“Keith”:“Stephen”;
...
<div>
<div style={{fontSize:11,color:nextTeam===“A”?CA:CB,fontWeight:700}}>{nextCaptain} is on the clock — Pick #{totalPicked+1}</div>
...
return<div key={name} style={{display:“flex”,justifyContent:“space-between”,alignItems:“center”,padding:“4px 0”,borderBottom:“1px solid #111”}}>
<span style={{fontSize:10,color:isCap?”#c9a227”:”#ccc”,fontWeight:isCap?700:400}}>{i+1}. {name}{isCap?” ⚑”:””}</span>
{isCap?<span style={{fontSize:8,color:”#4a3200”}}>LOCKED</span>:<span onClick={()=>assign(name,null)} style={{color:”#333”,cursor:“pointer”,fontSize:14}}>×</span>}
</div>;
})}
</div>
))}
...
{unassigned.length>0&&(
<>
<div style={{fontSize:10,color:”#555”,fontWeight:700,letterSpacing:2,marginBottom:8}}>AVAILABLE PLAYERS</div>
{unassigned.map(name=>{
const avail=[“Drew”,“Alex”,“Bobby”,“Harry”].includes(name)?“Fri only”:name===“Mike Barr”?“Fri+maybe Thu”:“Both days”;
const isNext=nextTeam;
return<div key={name} style={{display:“flex”,alignItems:“center”,background:”#0a0a0a”,border:“1px solid #1a1a1a”,borderRadius:8,padding:“10px 12px”,marginBottom:6,gap:10}}>
<div style={{flex:1}}>
<div style={{fontSize:13,color:”#ddd”,fontWeight:700}}>{name}</div>
<div style={{fontSize:9,color:”#444”,marginTop:2}}>{avail}</div>
</div>
<button onClick={()=>assign(name,“A”)} style={{background:CA+“22”,border:“1px solid “+CA+(isNext===“A”?“99”:“33”),color:CA,borderRadius:5,padding:“5px 12px”,fontSize:11,fontWeight:700,cursor:“pointer”,boxShadow:isNext===“A”?“0 0 8px “+CA+“44”:“none”}}>A</button>
<button onClick={()=>assign(name,“B”)} style={{background:CB+“22”,border:“1px solid “+CB+(isNext===“B”?“99”:“33”),color:CB,borderRadius:5,padding:“5px 12px”,fontSize:11,fontWeight:700,cursor:“pointer”,boxShadow:isNext===“B”?“0 0 8px “+CB+“44”:“none”}}>B</button>
</div>;
})}
</>
)}
...
// FIX #9: confirmation before reset
...
<div>
<div style={{fontSize:13,color:”#ddd”,fontWeight:700}}>{user}</div>
<div style={{fontSize:9,color:”#444”,marginTop:2}}>{uTeam?(uTeam===“A”?st.tA:st.tB):“Unassigned”}{isCap?” · Captain ⚑”:””}</div>
</div>
...
{!confirmReset
?<button onClick={()=>setConfirmReset(true)} style={{width:“100%”,background:”#1a0808”,border:“1px solid #2a1010”,borderRadius:6,color:”#c0392b”,fontSize:11,fontWeight:700,padding:“8px”,cursor:“pointer”}}>↺ Reset Teams & Matchups</button>
:<div style={{background:”#1a0808”,border:“1px solid #c0392b”,borderRadius:8,padding:12}}>
<div style={{fontSize:12,color:”#ddd”,marginBottom:10,textAlign:“center”}}>⚠️ This will wipe all teams and scores. Are you sure?</div>
<div style={{display:“flex”,gap:8}}>
<button onClick={()=>{save({…st,pA:[“Keith”],pB:[“Stephen”],teamsSet:false,thu:[],fri:[]});setConfirmReset(false);}} style={{flex:1,background:”#c0392b”,border:“none”,borderRadius:5,color:”#fff”,fontSize:12,fontWeight:700,padding:“8px”,cursor:“pointer”}}>Yes, Reset</button>
...
const pushN=(ns,msg,type)=>{const n={id:Date.now()+Math.random(),msg,ts:Date.now(),t:type};setToast(n);setTimeout(()=>setToast(t=>t?.id===n.id?null:t),6000);return{…ns,notifs:[…(ns.notifs||[]),n]};};
...
const isScr=!isThu||isThuScramble(true,hole);
const arr=[…st[day]];const mm={…arr[mi]};const sd={…mm[side]};
const k=team===“A”?“sA”:“sB”;
if(isScr){
// scramble: store under all players in pair (or just pl if solo)
const up={…sd[k]};
(pair||[pl]).forEach(p=>{
if(score===null){const copy={…(up[p]||{})};delete copy[hole];up[p]=copy;}
else{up[p]={…(up[p]||{}),[hole]:score};}
});
sd[k]=up;
} else {
// FIX #5: allow clearing individual scores
...
let a=0,b=0,ties=0;
hs.forEach(h=>{const scr2=!isThu||isThuScramble(true,h.h);const ha=getScore(upd.sA,upd.pA,h.h,scr2),hb=getScore(upd.sB,upd.pB,h.h,scr2);if(ha&&hb){if(ha<hb)a++;else if(hb<ha)b++;else ties++;}});
...
// FIX #7: auto-jump to user’s match on first render
...
{uMi>=0&&uMi!==ci&&(
<div onClick={()=>{setMi(uMi);setSide(“front”);}} style={{fontSize:10,color:”#c9a227”,cursor:“pointer”,marginBottom:6,fontWeight:700,display:“flex”,alignItems:“center”,gap:4}}>
<span>⚑</span> Jump to my match (M{uMi+1})
</div>
)}
...
{isThu
?<ThuCard holes={hs} pA={sd.pA||[]} pB={sd.pB||[]} sA={sd.sA||{}} sB={sd.sB||{}} me={user} onScore={(p,h,s,t,isScr)=>handleScore(p,h,s,t,isScr?sd.pA.includes(p)?sd.pA:sd.pB:null,day)}/>
